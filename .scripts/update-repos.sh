...
REPO_LIST=$(gh repo list devopshost --visibility public --source --limit 5 --sort pushed --json name,description,url --jq '.[] | "- [**\(.name)**](\(.url)) - \(.description // "No description provided.")"')
